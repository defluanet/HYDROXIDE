...
function Library:UpdateKeybindFrame()
    ...
    for _, KeybindToggle in pairs(Library.KeybindToggles) do
        ...
        VisibleKeybindCount = VisibleKeybindCount + 1
    ...
    Library.KeybindFrame.Visible = shouldShow
    if shouldShow then
        Library.KeybindFrame.Size = UDim2.fromOffset(XSize + 18 * Library.DPIScale, 0)
    end
end

function Library:UpdateStatusFrame()
    if not Library.StatusFrame then
        return
    end

    local XSize = 0
    local StatusItemCount = 0

    for _, item in pairs(Library.StatusContainer:GetChildren()) do
        if item:IsA("TextLabel") then
            StatusItemCount = StatusItemCount + 1
            local textBounds = game:GetService("TextService"):GetTextSize(
                item.Text,
                item.TextSize,
                item.Font,
                Vector2.new(math.huge, item.Size.Y.Offset)
            )
            if textBounds.X > XSize then
                XSize = textBounds.X
            end
        end
    end

    local shouldShow = Library.StatusFrameEnabled
    if Library.HideInactiveStatus and StatusItemCount == 0 then
        shouldShow = false
    end

    Library.StatusFrame.Visible = shouldShow
    if shouldShow then
        local finalWidth = math.max(XSize + 18 * Library.DPIScale, 150 * Library.DPIScale)
        Library.StatusFrame.Size = UDim2.fromOffset(finalWidth, 0)
    end
end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:Update()
            ...
            -- Sync toggle state with parent object
            ...
            if KeybindsToggle.Loaded then
                ...
                if KeyPicker.Value == "None" then
                    ShouldShow = false
                elseif Library.HideInactiveKeybinds then
        ...
        Picker.MouseButton1Click:Connect(function()
            ...
            repeat
                ...
                -- In Press mode, treat modifiers as normal keys
                if IsModifierInput(Input) and KeyPicker.Mode ~= "Press" then
...
do
    ...
    function Funcs:AddActionList(Params)
        Params = Params or {}
        local Groupbox = self
        local Container = Groupbox.Container
        ...
        local Frame = New("Frame", {
            ...
            Size = UDim2.new(1, 0, 0, ActionList.Height),
            Parent = Container,
        })
        ...
        local function CreateActionEntry(index, action, isKeybind)
            local text = ""
            if action.type == "equip" then
                text = index .. ". <font color=\"#7d55ff\">Equip</font>: " .. (action.tool or "?")
            elseif action.type == "unequip" then
                text = index .. ". <font color=\"#ff5555\">Unequip</font>"
            elseif action.type == "wait" then
                text = index .. ". <font color=\"#ffaa00\">Wait</font>: " .. string.format("%.2fs", action.duration or 0)
            elseif action.type == "activate" then
                text = index .. ". <font color=\"#55ffff\">Activate</font>"
            elseif action.type == "leftclick" then
                text = index .. ". <font color=\"#00ff00\">Left Click</font>"
            elseif action.type == "rightclick" then
                text = index .. ". <font color=\"#00ffaa\">Right Click</font>"
            elseif action.type == "charge" then
                text = index .. ". <font color=\"#5555ff\">Charge Mana</font>"
            elseif action.type == "stopcharge" then
                text = index .. ". <font color=\"#ff55ff\">Stop Charge</font>"
            elseif action.type == "chargeuntil" then
                text = index .. ". <font color=\"#55aaff\">Charge Until</font>: " .. (action.amount or 50)
            elseif action.type == "nostun" then
                text = index .. ". <font color=\"#ff8800\">No Stun</font>: " .. string.format("%.2fs", action.duration or 0.2)
            elseif action.type == "punch" then
                text = index .. ". <font color=\"#ffcc00\">Punch</font>"
            elseif action.type == "heavypunch" then
                text = index .. ". <font color=\"#ffaa00\">Heavy Punch</font>"
            elseif action.type == "bufferaction" then
                text = index .. ". <font color=\"#ff00ff\">Buffer Action</font>"
            elseif action.type == "waitstun" then
                text = index .. ". <font color=\"#ff5555\">Wait Stun</font>"
            elseif action.type == "loopclick" then
                text = index .. ". <font color=\"#00ffff\">Loop Click</font>: " .. string.format("%.2fs", action.duration or 1)
            elseif action.type == "keybind" then
                if isKeybind then
                    text = index .. ". <font color=\"#ffdd00\">" .. tostring(action.key or "?") .. "</font> -> " .. tostring(action.tool or "?")
                else
                    text = index .. ". <font color=\"#ffdd00\">Keybind</font>: " .. tostring(action.key or "?") .. " -> " .. tostring(action.tool or "?")
                end
            elseif action.type == "keyclick" then
                text = index .. ". <font color=\"#44ffbb\">Key Click</font>: " .. string.format("0x%02X", action.keycode or 0)
            elseif action.type == "keypress" then
                text = index .. ". <font color=\"#44ddff\">Key Press</font>: " .. string.format("0x%02X", action.keycode or 0)
            elseif action.type == "keyrelease" then
                text = index .. ". <font color=\"#44bbff\">Key Release</font>: " .. string.format("0x%02X", action.keycode or 0)
            elseif action.type == "executecode" then
                text = index .. ". <font color=\"#ff00aa\">Execute</font>: " .. (action.file or "?")
            else
                text = index .. ". " .. tostring(action.type)
            end
        ...
        function ActionList:SetActions(actions)
            ActionList.Actions = actions or {}
            for _, child in ipairs(Scroll:GetChildren()) do
                if child:IsA("TextLabel") then
                    child:Destroy()
                end
            end
            if #ActionList.Actions == 0 then
                New("TextLabel", {
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, 16),
                    Text = "<font color=\"#888888\">No actions</font>",
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Scroll,
                })
            else
                -- Separate regular actions from keybind actions
                local regular_actions = {}
                local keybind_actions = {}

                for _, action in ipairs(ActionList.Actions) do
                    if action.type == "keybind" then
                        table.insert(keybind_actions, action)
                    else
                        table.insert(regular_actions, action)
                    end
                end

                -- Display regular actions first
                for i, action in ipairs(regular_actions) do
                    CreateActionEntry(i, action, false)
                end

                -- Add keybinds section if any exist
                if #keybind_actions > 0 then
                    ...
                    New("TextLabel", {
                        BackgroundTransparency = 1,
                        Size = UDim2.new(1, 0, 0, 16),
                        Text = "<font color=\"#aaaaaa\">- Keybinds</font>",
                        TextSize = 13,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        Parent = Scroll,
                    })

                    -- Display keybind actions
                    for i, action in ipairs(keybind_actions) do
                        CreateActionEntry(i, action, true)
                    end
                end
            end
        end

        function ActionList:SetVisible(Visible)
            ActionList.Visible = Visible
            Frame.Visible = Visible
            Groupbox:Resize()
        end

        ActionList:SetActions(ActionList.Actions)

        Groupbox:Resize()
        ActionList.Frame = Frame
        ActionList.Holder = Frame  -- For search compatibility
        ActionList.Scroll = Scroll
        table.insert(Groupbox.Elements, ActionList)
...
function Library:Notify(...)
    ...
    task.delay(Library.NotifyTweenInfo.Time, function()
        ...
        elseif typeof(Data.Time) == "Instance" then
            ...
            local tweenTime = tonumber(Data.Time) or 5 -- Default to 5 seconds if invalid
            TweenService
                :Create(TimerFill, TweenInfo.new(tweenTime, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        Library.StatusFrame, Library.StatusContainer = Library:AddDraggableMenu("Status Effects")
        Library.StatusFrame.AnchorPoint = Vector2.new(0, 0)
        Library.StatusFrame.Position = UDim2.new(0, 6, 0, 46)
        Library.StatusFrame.Visible = false
        Library:UpdateDPI(Library.StatusFrame, {
            Position = false,
            Size = false,
        })
...
function Library:StatusWindow(windowProperties)
    local statusWindow = {
        statusItems = {},
        frame = Library.StatusFrame,
        container = Library.StatusContainer,

        SetVisible = function(self, visible)
            Library.StatusFrameEnabled = visible
            Library:UpdateStatusFrame()
        end,

        AddItem = function(self, text, color)
            local textLabel = New("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 20),
                Text = text,
        ...
        Clear = function(self)
            for _, item in pairs(self.statusItems) do
                if item.label then
                    item.label:Destroy()
                end
            end
        ...
        UpdatePosition = function(self, x, y)
            if self.frame then
                self.frame.Position = UDim2.new(x / 100, 0, y / 100, 0)
            end
        end
    }
...
return function(shared, utility)
    if shared then
        if shared.theme then
            Library.Scheme.AccentColor = shared.theme.accent or Library.Scheme.AccentColor
        end
    end
